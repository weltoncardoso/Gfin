...
class Nfe extends model {
	...
	public function emitirNFE($cNF, $destinatario, $prods, $fatinfo) {
		...
		$tpImp = '4'; //0=Sem geração de DANFE; 1=DANFE normal, Retrato; 2=DANFE normal, Paisagem; 3=DANFE Simplificado; 4=DANFE NFC-e; 5=DANFE NFC-e em mensagem eletrônica
		...
		foreach($prods as $pchave => $prod) {
		   ...
		   // $vBC = $prod['vBC'];
		    ...
		    $nfe->tagICMSSN($nItem, $orig, $csosn, $modBC, $pRedBC, $pICMS, $vICMS, $pCredSN, $vCredICMSSN, $modBCST, $pMVAST, $pRedBCST, $vBCST, $pICMSST, $vICMSST, $vBCSTRet, $vICMSSTRet);
			...
			$vTotOutro += $vOutro;
			$vTotTrib += $vTrib;
			$vTotal += ($vProd+$vFrete+$vSeg-$vDesc+$vOutro+$vTrib);
		}
		...
		if ($mod == 55) {
			...
			$nfe->tagpag(99, $fatinfo['vorig'] - $fatinfo['vdesc']); //deve ir no formato 0.00
		}
// Monta a NF-e e retorna o resultado
		...
		if($resp === true) {
			...
			if ($mod == 55) {
				...
				$danfce = new  NFePHP\Extras\Danfce($docxml, $docLogo, 2);
				...
				$salva = $danfce->printDANFCE('pdf', $pathDanfeFile, 'F'); //Salva na pasta pdf
